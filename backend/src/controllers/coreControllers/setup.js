...
const setup = async (req, res) => {
  ...
  for (const filePath of settingsFiles) {
    ...
    const settingsToUpdate = {
      nexora_app_email: email,
      nexora_app_company_email: email,
      nexora_app_timezone: timezone,
      nexora_app_country: country,
      nexora_app_language: language || 'en_us',
    };
  ...
  return res.status(200).json({
    success: true,
    result: {},
    message: 'Successfully Nexora Admin Setup', // Changed from IDURAR
  });
};
